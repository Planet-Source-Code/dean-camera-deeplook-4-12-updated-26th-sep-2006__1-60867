...
Begin VB.Form FrmSelProject 
   ...
   Begin VB.Label lblLocateProjText 
      BackStyle       =   0  'Transparent
      Caption         =   "Please enter the filename of the Visual Basic file to be scanned:"
      ForeColor       =   &H00000000&
      Height          =   255
      Left            =   840
      TabIndex        =   1
...
Private Sub btnBrowseButton_Click()
    cdgDialogs.Filter = "VB6 and .NET Projects (*.vbp, *.vbg, *.vbproj)|*.vbp;*.vbg;*.vbproj|Visual Basic 6 Project Files (*.vbp *.vbg)|*.vbp;*.vbg|Visual Basic 6 File|*.frm;*.bas;*.cls;*.ctl;*.pag;*.dob;*.dsr|.NET Project (*.vbproj)|*.vbproj|DeepLook Project Report (*.txt)|*.txt|DeepLook Prescanned File (*.dst)|*.dst"
    cdgDialogs.ShowOpen
...
Private Sub Form_Load()
    ...
    lblLocateProjText.Caption = "Please enter the filename of the Visual Basic file to be scanned:"
...
Private Sub cmbProjectPath_Change()
    ...
    If Len(cmbProjectPath.Text) > 3 Then
        If Right$(cmbProjectPath.Text, 4) = ".txt" Then
            btnGoAnalyse.Caption = "Convert to XML"
        Else
            btnGoAnalyse.Caption = "Analyse!"
        End If
    End If
End Sub
...
Public Sub GetScannedFilesFromRegistry()
    ...
    With cmbProjectPath.ComboItems
        ...
        .Add , , "<< Clear Recent Scanned Project/Files List >>", "Clean", , 1
    End With
End Sub
...
Private Sub cmbProjectPath_Click()
    ...
    If cmbProjectPath.Text = "<< Clear Recent Scanned Project/Files List >>" Then
        RetVal = MsgBox("Are you sure you want to clear the recent scanned project/file list?", vbQuestion + vbYesNo + vbDefaultButton2, "DeepLook")
...
Public Sub GoAnalyse()
    ...
    If cmbProjectPath.Text = "<< Clear Recent Scanned Project/Files List >>" Then Exit Sub
    ...
    If LinesPerSec < 1 Then LPSData = "" Else LPSData = " (" & LinesPerSec & " lines/sec)" ' Fix up the lines per second string

    DefaultStatText = "Scan took " & TimeTemp & " second" & AddS & LPSData & ". This is DeepLook version " & App.Major & "." & App.Minor & "." & App.Revision & "." ' Finish the status bar text string
    ...
    If PROJECTMODE = VB6 Then ' DeepLook can only copy required files for VB6 files/projects
        ...
        With FrmResults.lstVarList
            ...
            .ListItems.Add , , ""
            .ListItems.Add , , "Total: " & I ' Add the total number of found unused variables
    ...
    With FrmResults
        If .TreeView.Nodes.Count > 16000 Then ' Show large number of nodes warning in the treeview
            .TreeView.Nodes.Add 1, tvwChild, "NodeWarning", "Warning: There are a large amount (" & Format$(FrmResults.TreeView.Nodes.Count, "###,###,###") & ") of nodes in the treeview.", "Warning"
            .TreeView.Nodes.Add 1, tvwChild, "NodeWarningP2", "This large number may cause DeepLook or your computer to run slowly or stop responding."
        End If
